...
import (
	...
	configdoc "github.com/bamanoz/tabula/internal/config"
...
func loadTOMLDocument(path string) (configdoc.Document, error) {
	var doc map[string]any
	if _, err := toml.DecodeFile(path, &doc); err != nil {
		return nil, fmt.Errorf("load config %s: %w", path, err)
	}
	if doc == nil {
		return configdoc.Document{}, nil
	}
	return configdoc.Document(doc), nil
}

func pluginSubdocument(doc configdoc.Document, pluginID string) configdoc.Document {
...
func mergeDocuments(base, overlay configdoc.Document) configdoc.Document {
...
func documentValue(value any) (configdoc.Document, bool) {
	switch typed := value.(type) {
	case configdoc.Document:
		return typed, true
	case map[string]any:
		return configdoc.Document(typed), true
	case map[string]map[string]any:
		out := configdoc.Document{}
		for key, item := range typed {
			out[key] = configdoc.Document(item)
		}
		return out, true
	default:
		return nil, false
...
func cloneDocument(in map[string]any) configdoc.Document {
	out := configdoc.Document{}
