...
const (
	...
	strategyClaiming  strategy = "claiming"
)
...
func (e *Engine) SetSessionTenantResolver(resolve func(string, string) string) {
	if e == nil {
		return
	}
...
func (e *Engine) SetAuditRecorder(record func(DispatchAudit)) {
	if e == nil {
		return
	}
	e.audit = record
...
func (e *Engine) SetDefaultTimeout(timeout time.Duration) {
	if e == nil {
		return
	}
...
func (e *Engine) RebuildIndex(subs []Subscriber) {
...
func (e *Engine) Dispatch(event string, payload json.RawMessage, tenantID, session string) (json.RawMessage, bool) {
	result, ok, _ := e.DispatchDetailedExcept(event, payload, tenantID, session, nil)
	return result, ok
}

func (e *Engine) DispatchExcept(event string, payload json.RawMessage, tenantID, session string, exclude Subscriber) (json.RawMessage, bool) {
	result, ok, _ := e.DispatchDetailedExcept(event, payload, tenantID, session, exclude)
	return result, ok
}

func (e *Engine) DispatchDetailedExcept(event string, payload json.RawMessage, tenantID, session string, exclude Subscriber) (json.RawMessage, bool, *DispatchDecision) {
	entries := e.entries(event)
	if len(entries) == 0 {
		e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Status: "missing", DispatchEffect: "continued", Payload: payload})
	...
	for _, entry := range entries {
		...
		if !entry.sub.IsConnected() {
			if eventType == Security && unavailableSecurityTarget == "" {
		...
		if entry.sub.IsBusy() && eventType != Security && hookBusyPolicy(event) != BusyWait && ReplyMode(event) != wire.HookReplyModeNone {
			continue
		}
		...
		} else if eventType == Security && unavailableSecurityTarget == "" {
			unavailableSecurityTarget = entry.sub.Name()
		}
	}
	if eventType == Security && len(skipped) > 0 {
		e.releaseReservedHooks(relevant)
		entry := skipped[0]
		e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.entry.sub.Name(), Status: entry.status, DispatchEffect: "failed_closed", Payload: payload})
		return nil, false, &DispatchDecision{Event: event, Target: entry.entry.sub.Name(), Status: entry.status}
	}
	if len(relevant) == 0 {
		if eventType == Security && unavailableSecurityTarget != "" {
			e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: unavailableSecurityTarget, Status: "unavailable", DispatchEffect: "failed_closed", Payload: payload})
			return nil, false, &DispatchDecision{Event: event, Target: unavailableSecurityTarget, Status: "unavailable"}
		}
		e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Status: "unavailable", DispatchEffect: "continued", Payload: payload})
		return payload, true, nil
	}
	...
	case strategyModifying:
		return e.dispatchModifying(event, payload, tenantID, session, relevant, def.Type == Security)
...
func (e *Engine) reserveHookEntry(entry hookEntry, event string) (hookEntry, bool, string) {
	if ReplyMode(event) == wire.HookReplyModeNone {
		return entry, true, ""
	}
	runtimeSub, ok := entry.sub.(BusySubscriber)
	if !ok {
		return entry, true, ""
	}
	if e.eventType(event) == Security || hookBusyPolicy(event) == BusyWait {
...
func (e *Engine) waitRuntimeHookEntry(entry hookEntry, runtimeSub BusySubscriber, event string) (hookEntry, bool, string) {
...
func hookBusyPolicy(event string) BusyPolicy {
	if def, ok := Events[event]; ok {
		return def.BusyPolicy
	}
	return BusySkip
}

func (e *Engine) releaseReservedHooks(entries []hookEntry) {
	for _, entry := range entries {
...
// ReleaseDecision releases the busy reservation held by a suspended decision.
func ReleaseDecision(decision *DispatchDecision) {
...
func (e *Engine) eventType(event string) EventType {
	if def, ok := Events[event]; ok {
		return def.Type
	}
	return Security
...
func (e *Engine) CanHandleResult(sender Subscriber, id string) bool {
	pending, ok := e.pendingHook(id)
	return ok && pending.subscriber == sender
}

func (e *Engine) HandleRuntimeResult(runtimeID string, msg *Message) {
...
func (e *Engine) HandleResult(sender Subscriber, msg *Message) {
	pending, ok := e.pendingHook(msg.ID)
	if !ok {
		return
	}
...
func (e *Engine) deliverResult(pending pendingHook, msg *Message) {
	payload := msg.Payload
	if payload == nil {
		payload = msg.Data
	}
...
func (e *Engine) sessionTenantID(tenantID, session string) string {
	if tenantID != "" {
		return tenantID
	}
...
func (e *Engine) dispatchVoid(event string, payload json.RawMessage, tenantID, session string, entries []hookEntry) {
	for _, entry := range entries {
		entry.sub.SendHook(&Message{
			Type:     "hook",
			ID:       GenerateID(),
			Name:     event,
			Session:  session,
...
func (e *Engine) dispatchModifying(event string, payload json.RawMessage, tenantID, session string, entries []hookEntry, secure bool) (json.RawMessage, bool, *DispatchDecision) {
	return e.dispatchModifyingFrom(event, payload, tenantID, session, entries, secure)
}

func (e *Engine) ResumeModifying(decision *DispatchDecision) (json.RawMessage, bool, *DispatchDecision) {
...
func (e *Engine) reserveContinuationEntries(event string, payload json.RawMessage, tenantID, session string, entries []hookEntry, secure bool) ([]hookEntry, *DispatchDecision) {
	...
	for _, entry := range entries {
		if !entry.sub.IsConnected() {
			if secure {
				...
				e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), Status: "disconnected", DispatchEffect: "failed_closed", Payload: payload})
				return nil, &DispatchDecision{Event: event, Target: entry.sub.Name(), Status: "disconnected"}
			}
			continue
		}
		if !entry.sub.ServesTenant(tenantID) || (entry.sub.Session() != session && entry.sub.Session() != "") {
			if secure {
				e.releaseReservedHooks(reserved)
				e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), Status: "unavailable", DispatchEffect: "failed_closed", Payload: payload})
				return nil, &DispatchDecision{Event: event, Target: entry.sub.Name(), Status: "unavailable"}
			}
			continue
		}
		if duplicateRuntimeHookTarget(entry, seenRuntimeTargets) {
		...
		if !ok {
			if secure {
				e.releaseReservedHooks(reserved)
				e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), Status: status, DispatchEffect: "failed_closed", Payload: payload})
				return nil, &DispatchDecision{Event: event, Target: entry.sub.Name(), Status: status}
			}
			continue
		}
...
func (e *Engine) dispatchModifyingFrom(event string, payload json.RawMessage, tenantID, session string, entries []hookEntry, secure bool) (json.RawMessage, bool, *DispatchDecision) {
	...
	for i := range entries {
		...
		if result == nil {
			e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), HookID: outcome.hookID, Status: outcome.status, DispatchEffect: mapHookNilEffect(secure), DurationMs: outcome.durationMs, TimeoutMs: outcome.timeoutMs, Payload: current})
			if secure {
				e.logger.Info("security hook timeout, blocking", hookLogAttrs(event, entry.sub.Name(), current, tenantID, session)...)
				return nil, false, &DispatchDecision{Event: event, Target: entry.sub.Name(), HookID: outcome.hookID, Status: outcome.status}
			}
			continue
		}
		...
		case ActionSuspend:
			...
			blocked := *entry
			entry.release = nil
			return current, false, &DispatchDecision{Event: event, Target: blocked.sub.Name(), HookID: outcome.hookID, ReplyAction: result.Action, Reason: result.Reason, Status: outcome.status, Payload: append(json.RawMessage(nil), result.Payload...), Pending: true, continuation: &hookContinuation{event: event, tenantID: tenantID, session: session, current: append(json.RawMessage(nil), current...), blocked: &blocked, entries: remaining, secure: secure}}
		case ActionBlock:
			e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), HookID: outcome.hookID, ReplyAction: result.Action, DispatchEffect: "tool_not_invoked", Reason: result.Reason, Status: "reply", DurationMs: outcome.durationMs, TimeoutMs: outcome.timeoutMs, Payload: current})
			attrs := hookLogAttrs(event, entry.sub.Name(), current, tenantID, session)
			attrs = append(attrs, "reason", result.Reason)
			e.logger.Info("hook blocked event", attrs...)
			return nil, false, &DispatchDecision{Event: event, Target: entry.sub.Name(), HookID: outcome.hookID, ReplyAction: result.Action, Reason: result.Reason, Status: outcome.status, Payload: append(json.RawMessage(nil), result.Payload...)}
		case ActionModify:
			e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), HookID: outcome.hookID, ReplyAction: result.Action, DispatchEffect: "payload_rewritten", Reason: result.Reason, Status: "reply", DurationMs: outcome.durationMs, TimeoutMs: outcome.timeoutMs, Payload: current})
			if result.Payload != nil {
				current = e.mergePayload(event, current, result.Payload)
			}
		default:
			e.recordAudit(DispatchAudit{Event: event, TenantID: tenantID, Session: session, Target: entry.sub.Name(), HookID: outcome.hookID, ReplyAction: result.Action, DispatchEffect: "continued", Reason: result.Reason, Status: "reply", DurationMs: outcome.durationMs, TimeoutMs: outcome.timeoutMs, Payload: current})
		}
	}
...
// payload.
func (e *Engine) mergePayload(event string, prev, next json.RawMessage) json.RawMessage {
...
func (e *Engine) dispatchClaiming(event string, payload json.RawMessage, tenantID, session string, entries []hookEntry) (json.RawMessage, bool) {
	defer e.releaseReservedHooks(entries)
	for i := range entries {
		entry := &entries[i]
...
func (e *Engine) sendAndWait(entry *hookEntry, event string, payload json.RawMessage, tenantID, session string) hookSendOutcome {
	s := entry.sub
	id := GenerateID()
...
func (e *Engine) recordAudit(event DispatchAudit) {
...
func (e *Engine) injectSession(payload json.RawMessage, tenantID, session string) json.RawMessage {
	tenantID = e.sessionTenantID(tenantID, session)
	if session == "" && tenantID == "" {
		return payload
	}
...
// Subscribers returns a snapshot of subscribers indexed for event.
func (e *Engine) Subscribers(event string) []Subscriber {
	entries := e.entries(event)
...
func (e *Engine) entries(event string) []hookEntry {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func (e *Engine) addPendingHook(id string, subscriber Subscriber, ch chan *Result) {
	e.mu.Lock()
	defer e.mu.Unlock()
	pending := pendingHook{subscriber: subscriber, ch: ch}
	if runtimeSub, ok := subscriber.(BusySubscriber); ok {
		pending.runtimeID = runtimeSub.RuntimeID()
	}
	e.pending[id] = pending
}

func (e *Engine) setPendingHookRelease(id string, release func()) {
	if release == nil {
		return
	}
	e.mu.Lock()
...
func (e *Engine) AddPendingRuntimeHook(id string, runtimeID string, ch chan *Result) {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *Engine) pendingHook(id string) (pendingHook, bool) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	ch, ok := e.pending[id]
	return ch, ok
