...
func TestBarePolicySpawnInitCallAndShutdown(t *testing.T) {
	...
	if err := worker.Init(context.Background(), workerwire.WorkerInit{KernelID: req.KernelID, TenantID: req.TenantID, TargetID: req.TargetID, Manifest: req.Manifest}); err != nil {
		t.Fatalf("Init: %v", err)
	}
...
func TestInitTimeoutKillsUnresponsiveWorker(t *testing.T) {
	...
	if err := worker.Init(context.Background(), workerwire.WorkerInit{KernelID: "main", TenantID: "default", TargetID: "fs"}); err == nil {
...
for line in sys.stdin:
    ...
    sys.stdout.write(json.dumps({"call_id": call_id, "ok": True, "data": data}) + "\n")
    sys.stdout.flush()
`
