...
func TestWriteLocalRuntimeConfig_RoundTrip(t *testing.T) {
	...
	if err := writeLocalRuntimeConfig(tabulaHome, []plugin.BootEntry{{ManifestPath: second}, {ManifestPath: first}, {ManifestPath: second}}); err != nil {
...
func TestLocalRuntimePluginPaths_RejectsBlankManifestPath(t *testing.T) {
	_, err := localRuntimePluginPaths(t.TempDir(), []plugin.BootEntry{{ManifestPath: "  "}})
	if err == nil {
...
func TestManagedLocalRuntimePIDAndWaitForAttachment(t *testing.T) {
	...
	if err := runtimeProc.WaitForAttachment(attached.Load, time.Second); err != nil {
		t.Fatalf("WaitForAttachment: %v", err)
	}
	if err := runtimeProc.Shutdown(time.Second); err != nil {
...
func TestManagedLocalRuntimeWaitForAttachmentFailsWhenProcessExits(t *testing.T) {
	...
	runtimeProc := newManagedLocalRuntime(cmd)
	err := runtimeProc.WaitForAttachment(func() bool { return false }, time.Second)
	if err == nil {
