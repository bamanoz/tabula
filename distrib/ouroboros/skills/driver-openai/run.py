...
def log(msg: str) -> None:
    if VERBOSE:
        sys.stderr.write(f"[ouroboros:driver-openai] {msg}\n")
        sys.stderr.flush()
...
def effective_model(default_model: str, session: str) -> str:
    ...
    if override.get("provider") == "openai" and override.get("model"):
        return str(override["model"])
    return default_model


def main() -> None:
    parser = argparse.ArgumentParser(description="Ouroboros LLM driver (OpenAI)")
