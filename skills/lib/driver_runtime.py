...
class DriverRuntime:
    ...
    def process_turn(self, suppress_stream: bool = False):
        ...
        try:
            ...
            if self.collecting:
                ...
                return
            self.conn.send({"type": "stream_start"})
            self.conn.send({"type": "stream_delta", "text": f"[API Error: {exc}]"})
    ...
    def _flush_collected_internal(self, *, timeout: bool):
        ...
        for result in self._collected:
            parts.append(f"[Result from subagent {result['id']}]:\n{result['text']}")
        ...
        if timeout and remaining:
            for agent_id in sorted(remaining):
                parts.append(f"[Result from subagent {agent_id}]:\n[subagent timed out waiting for result]")
    ...
    def handle_tool_result(self, msg: dict):
        ...
        if pid_match and tool_id in self._tool_to_spawn_id:
            ...
            self._record_subagent_result(agent_id, f"[spawn failed via {tool_id}: {output}]")
    ...
    def handle_error(self, msg: dict):
        ...
        if match:
            ...
            if agent_id and agent_id in self._known_subagent_ids:
                self._record_subagent_result(agent_id, f"[subagent crashed: {text}]")
                return
        if self.collecting:
            return

        self.provider.add_user_text(f"[system error: {text}]")
        self.process_turn()
