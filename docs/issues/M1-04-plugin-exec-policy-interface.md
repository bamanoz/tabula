...
- `PluginExecPolicy` (`cmd/tabula-runtime/policy/policy.go`):
  - `Spawn(ctx, SpawnReq) (Worker, error)`

- `Worker` (`cmd/tabula-runtime/policy/worker.go`):
...
`cmd/tabula-runtime/policy/bare/`:
...
- [ ] `go build ./...` clean.
- [ ] `go test ./cmd/tabula-runtime/policy/...` green.
