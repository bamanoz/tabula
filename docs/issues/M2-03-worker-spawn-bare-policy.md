...
- `cmd/tabula-runtime/policy/bare/`: full `barePolicy.Spawn`
  implementation:
  ...
  - Send `WorkerInit`, await `WorkerInitAck` with timeout (10s
    default).
- `cmd/tabula-runtime/policy/bare/worker.go`: `Worker`
  implementation:
...
- `cmd/tabula-runtime/pool/`: worker pool keyed by
...
- `cmd/tabula-runtime/manifest/`: read `plugin.toml` files from
