...
- Config layering (per ADR §5):
  ...
  - Merge order: global ← tenant ← (future: session). Last
    ...
    godoc.
- `internal/kernel/config/`: extend loader to take a tenant_id
...
- [ ] Config loader takes tenant_id; merge order enforced via
      tests.
- [ ] Session create requires (or defaults) tenant_id; rejects
      unknown tenants with `tenant_unknown` error.
- [ ] Every `Invoke` envelope sent from kernel carries the
      session's tenant_id (verified via mock RuntimeConn
      assertions).
- [ ] Sessions stored under `tenants/<id>/state/sessions/`.
- [ ] Per-tenant plugin config is read; values from
      `config/plugins/foo/config.toml` correctly overlaid by
      `tenants/myproject/config/plugins/foo/config.toml`.
- [ ] SDK `plugin_state_dir()` returns tenant-scoped path when
      `TABULA_TENANT_DIR` is set; warns and falls back when
      missing.
- [ ] Existing single-tenant tests pass after migration to
