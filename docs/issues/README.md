...
- [ ] `workspace/fs` plugin owns every fs tool the agent
      uses; lexical roots enforcement, structured
      `fs_outside_root` errors.
- [ ] `workspace/exec` plugin owns shell execution;
      independent of fs.roots per Q3.4c; supports background
      processes.
- [ ] `${project_root}`, `${tenant_id}`, `${tabula_home}`
...
- [ ] `files/files`, `base/shell`, `code/workspace` skills
      physically deleted from `tabula-bundles`.
- [ ] `code/hook-workspace-boundary` migrated or deleted
      based on audit.
- [ ] Distros updated to pin fs/exec instead of legacy
      skills.
- [ ] Testbed suites cover roots, tenant divergence, exec
      independence, missing-project-root fallback.
- [ ] Cross-repo grep guards pass.
