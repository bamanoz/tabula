...
- [ ] All seven tools implemented and unit-tested.
- [ ] Path outside any root → `fs_outside_root` structured
      error.
- [ ] `..` traversal blocked (`/root/../etc/passwd` rejected).
- [ ] Default `follow_symlinks=false`: symlinked path that
      escapes root rejected.
- [ ] `${project_root}` config template resolved per tenant.
- [ ] Plugin survives reload (warm worker continues, new
      config picked up on next call after reload).
- [ ] Testbed suite green.
- [ ] No imports from the old `files` skill surface — fs is
      a standalone plugin.
