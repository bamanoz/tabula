...
- [ ] All four tools implemented and unit-tested.
- [ ] Timeout behavior verified (process SIGTERM at timeout,
      SIGKILL 5s later if still alive, clean `timed_out:
      true` result).
- [ ] Background processes survive across `exec_run` calls
      until killed or worker restarts.
- [ ] `${project_root}` and `${tenant_id}` config templates
      resolve correctly per tenant.
- [ ] Deny-list rejection is structured.
- [ ] No coupling to `fs` plugin (exec works standalone with
