...
Long-running skills connect over WebSocket using `skills/lib/kernel_client.py`.
...
## When to use `skills/lib/`

Use `skills/lib/` when it removes boilerplate and matches an existing pattern.

Good candidates:

- `kernel_client.KernelConnection`
- protocol constants from `skills.lib.protocol`
- path helpers from `skills.lib.paths`
...
1. **Wire protocol** (`internal/kernel/protocol.go`, `skills/lib/protocol.py`)
   — closest to stable
2. **assistant `SKILL.md` frontmatter contract** — mostly stable in practice,
   not yet explicitly versioned
3. **`skills/lib/` runtime API** — useful, but not yet a formally versioned
   public package

If you are writing reusable community skills, prefer depending on:

- the wire protocol
- a small subset of `skills/lib/` helpers and, if needed, the current
  assistant fallback `run.py tool <name>` convention
- a small subset of `skills/lib/` helpers

Do not assume every helper inside `skills/lib/` is permanent API.
