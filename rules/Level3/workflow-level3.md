...
> **TL;DR:** This document outlines a structured workflow for Level 3 (Intermediate Feature) tasks. These tasks involve developing significant new functionality that may span multiple components, requiring comprehensive planning, often necessitating targeted creative design phases, followed by systematic implementation, Phase 4.5 runtime QA validation, in-depth reflection, and feature-specific archiving. This workflow balances detailed process with efficiency for moderately complex features.
...
graph LR
    ...
    QA["5.5 RUNTIME QA<br>(4-5-qa Mode)"] -->
    ...
    QA -.-> QADocs["Runtime Evidence & QA Report"]
    ...
    style QA fill:#ffe0b2,stroke:#ff9800
...
```
  ...
  * **Milestone Checkpoint:**
    ...
    → If all YES: Proceed to Phase 5.5 Runtime QA.
    → If any NO: Complete implementation and testing.
    ```

### Phase 5.5: RUNTIME QA VALIDATION (QA Mode — `4-5-qa`)

QA is a dedicated Level 3 runtime-validation gate between BUILD and REFLECT. It records evidence and routes failures back to BUILD; it must not apply source fixes itself.

```mermaid
graph TD
    StartQA["Begin Phase 4.5 QA<br/>Runtime Validation"] --> ReadStatus["Read tasks.md<br/>Confirm BUILD: DONE"]
    ReadStatus --> RouteCategory["Route by Category<br/>visual / backend / deep / quick"]
    RouteCategory --> RunChecks["Run runtime checks<br/>browser / smoke / API / integration"]
    RunChecks --> WriteReport["Write QA report<br/>memory-bank/qa/qa-[task-id].md"]
    WriteReport --> Verdict{"Verdict?"}
    Verdict -- "PASSED/SKIPPED" --> QADone["Set QA: DONE<br/>Next: REFLECT"]
    Verdict -- "FAILED" --> Rebuild["Set BUILD: NOT_STARTED<br/>QA: NOT_STARTED<br/>Next: BUILD"]

    style StartQA fill:#ffe0b2,stroke:#ff9800
    style QADone fill:#a5d6a7,stroke:#81c784
    style Rebuild fill:#ef9a9a,stroke:#e57373
```

  * **Steps:**
    1. Confirm Phase Status is the canonical 7-line schema and `BUILD: DONE`.
    2. Route mechanically by `Category` through `4-5-qa`:
        - `visual` → browser/page/runtime checks via Playwright MCP when available.
        - `backend` → smoke command, API/CLI probe, and log review.
        - `deep` → visual + backend + integration-boundary evidence.
        - `quick` → minimal smoke path through the backend QA contract.
    3. Write/update `memory-bank/qa/qa-[task-id].md` and per-attempt artifacts under `memory-bank/qa/artifacts/[task-id]/attempt-[N]/`.
    4. On `PASSED` or degraded `SKIPPED`, set `QA: DONE` and route to REFLECT.
    5. On `FAILED`, preserve the four-status enum by setting `BUILD: NOT_STARTED`, `QA: NOT_STARTED`, and updating `QA Last Verdict`, `QA Attempts`, and `QA Last Report` metadata under `## Task Details`.
  * **Milestone Checkpoint:**
    ```
    ✓ L3 RUNTIME QA CHECKPOINT
    - BUILD: DONE confirmed before QA? [YES/NO]
    - Category-appropriate runtime checks attempted or explicitly degraded? [YES/NO]
    - QA report created at memory-bank/qa/qa-[task-id].md? [YES/NO]
    - Evidence paths recorded under memory-bank/qa/artifacts/ when applicable? [YES/NO]
    - Verdict and Phase Status action recorded in tasks.md? [YES/NO]

    → If PASSED/SKIPPED and QA: DONE: Proceed to Reflection.
    → If FAILED and attempts < 3: Return to BUILD with re-entry instructions.
    → If FAILED and attempts >= 3: Hard-stop for manual intervention.
    ```
