...
graph TD
    ...
    UpdateProgress --> Transition{"Next mode<br>by level?"}
    Transition -->|"Level 3"| NextQA["NEXT MODE:<br>QA MODE (4.5)"]
    Transition -->|"Level 4"| NextSecurity["NEXT MODE:<br>SECURITY MODE (4.7)"]
    Transition -->|"Level 1-2"| NextReflect["NEXT MODE:<br>REFLECT MODE"]
```
...
→ If all YES: Build complete - ready for QA mode (Level 3), SECURITY mode (Level 4), or REFLECT mode (Level 1-2)
...
→ NEXT RECOMMENDED MODE: QA MODE (Level 3), SECURITY MODE (Level 4), or REFLECT MODE (Level 1-2)
``` 
