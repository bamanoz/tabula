...
### BUILD → QA / SECURITY / REFLECT Transition

```markdown
### BUILD → QA / SECURITY / REFLECT
- Context preserved: Implementation status, challenges encountered, decisions
- Files transferred: tasks.md, progress.md, implementation notes, QA/SECURITY evidence roots when applicable
- Routing: Level 3 goes BUILD → QA → REFLECT; Level 4 goes BUILD → SECURITY → REFLECT; Level 1/2 go BUILD → REFLECT
- Rule optimization: Load QA, SECURITY, or reflection templates based on level and Phase Status completion
```
