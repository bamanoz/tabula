...
Call subagent `first_agent_name` via a NEW Task tool call with the EXACT template from `pipeline-prompt-templates.md` → "Template: First Agent (BUILD)". Replace variables only. Do NOT pass `task_id` for this normal call.
...
3. Call subagent `mid_agent_name` via a NEW Task tool call with that exact template text. Do NOT pass any previous `task_id` for this normal call.
4. `agents_called += 1`
