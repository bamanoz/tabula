...
11. Task tool `task_id` is scoped to ONE router call only. Never reuse a `task_id` from a previous phase, previous router, or previous loop iteration.
...
CRITICAL TASK SESSION RULE:
- Every normal router call in Step 6 MUST be a NEW Task tool call with no `task_id`.
- The saved `task_id` belongs ONLY to that exact target phase/router call.
- Never pass a saved `task_id` when the target phase changes, when the router changes, or when returning to Step 4/Step 5 for a new loop iteration.
- Example: if the target phase is `ARCHIVE`, call a NEW `6-archive` task. Do NOT resume any earlier `2-plan`, `seq-plan-*`, `4-build`, or other task session.
- Use `task_id` ONLY inside Step 7 retry rules for the same target phase/router call that just failed, timed out, returned empty, or left the target phase `NOT_STARTED`.
