...
Call subagent `seq-plan-l{L}-first` via Task tool with the EXACT template from `pipeline-prompt-templates.md` → "Template: First Agent (PLAN)". Replace variables only.
...
3. Call subagent `seq-plan-l{L}-mid` via Task tool with that exact template text
4. `agents_called += 1`
